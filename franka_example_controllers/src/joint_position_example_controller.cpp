...
void JointPositionExampleController::update(const ros::Time& /*time*/, // Update function to loop continuously to loop through
  ...
  ros::Duration duration_wait = ros::Duration(63.6+60*1);
  ...
  if (t < 0){
    ...
    update_arm((amplitude*g_out), joint);
    ROS_INFO("time: %f, angle: %f, joint: %f", t, (amplitude*g_out), joint);
  } 
    ...
    if (counter < 2) {
      ...
      elapsed_time_ = elapsed_time_ - duration_wait; 
      counter = 0;
    }
...
// void JointPositionExampleController::update_arm(const double g) {
//   for (size_t i = 0; i < 7; ++i) {
//       if (i == 4) {
//         position_joint_handles_[i].setCommand(initial_pose_[i] + g);
//       } 
